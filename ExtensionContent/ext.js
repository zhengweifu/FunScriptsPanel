...
function onImportCurves() {
	var data_string = $("#fun_import_curve_input").val();
	if(data_string) {
		var extScript = "$._ext_ILST.ImportAndExportCurves.import('" + data_string + "')";
		evalScript(extScript);
	} else {
		alert("Not found string value!");
	}
}

function onExportCurves() {
	var extScript = "$._ext_ILST.ImportAndExportCurves.export(3)";
	evalScript(extScript, function(result) {
        $('#fun_output_div').html(result);
    });
}
